...
load("datasets/evaluation-output.RData", envir=e.aggr)
load("datasets/patients-eval-output.RData", envir=env)
# load("datasets/models-eval-output.RData", envir=env)
...
stats = subset(bind_rows(test.stats.all, required.training.stats.all),
               Method %in% bestAggregators | Class=='Model')
...
b1 = ggplot(data=subset(stats, ((Class=="Model" & Subclass=="Original") | (Method %in%
        c("mean_5_(GD)_(min)_maj", "mean_5_(GG)_(lat)_maj", "mean_5_(YG)_(lat)_maj",
         "mean_renn_5_(GD)_(cen)_maj", "mean_renn_5_(GG)_(lat)_maj", "mean_renn_5_(YG)_(lat)_maj",
         "wmean_5_(GD)_(lat)_maj", "wmean_5_(GG)_(lat)_maj", "wmean_5_(YG)_(lat)_maj",
         "wmean_renn_5_(GD)_(lat)_maj", "wmean_renn_4_(GG)_(lat)_maj",
         "wmean_renn_4_(YG)_(lat)_maj")) )
       & Measure==PERFORMANCE.MEASURE),
       aes(x=ObscureLevel, y=Value, group=Method, linetype=ifelse(Class=='Model',"b","a"), colour=ifelse(Class=='Model',ifelse(Method=="OEA","oea","orig"),'knn'), shape=Subsubclass, fill=Subsubclass)) +
    geom_line() +
    ...
    # scale_color_manual(values=ref.colors) +
    # scale_fill_manual(values=ref.colors) +
    xlab("uncertainty level") +
    ylab(ifelse(PERFORMANCE.MEASURE=="Cost matrix", "total classification cost", PERFORMANCE.MEASURE)) +
    theme_bw() +
    ...
    coord_cartesian(xlim=c(0,0.51), ylim=c(50, ymax.val)) +
    scale_x_continuous(breaks=BREAKS.X)
...
stats = arrange(stats, Class,Subclass, Subsubclass)
...
aggrs.perf.measure1.1 = subset(stats, Measure==PERFORMANCE.MEASURE & Class!="Model" & Class %in% c("mean") & Subsubclass %in% c("GG", "YG", "GD") & Subclass!="CNN") %>%
    group_by(Class, Subclass, Subsubclass)

seleted = c("mean_5_(GD)_(min)_maj", "mean_5_(GG)_(lat)_maj", "mean_5_(YG)_(lat)_maj",
  "mean_renn_5_(GD)_(cen)_maj", "mean_renn_5_(GG)_(lat)_maj", "mean_renn_5_(YG)_(lat)_maj",
  "wmean_5_(GD)_(lat)_maj", "wmean_5_(GG)_(lat)_maj", "wmean_5_(YG)_(lat)_maj",
  "wmean_renn_5_(GD)_(lat)_maj", "wmean_renn_4_(GG)_(lat)_maj",
  "wmean_renn_4_(YG)_(lat)_maj")

aggrs.perf.measure1.2 = subset(stats, Measure==PERFORMANCE.MEASURE & Class!="Model" & Class %in% c("wmean") & Subsubclass %in% c("GG", "YG", "GD") & Subclass!="CNN") %>%
    group_by(Class, Subclass, Subsubclass)
# aggrs.perf.measure2.1 = subset(stats, Measure==PERFORMANCE.MEASURE & Class!="Model" & Class %in% c("median") & Subsubclass %in% c("GG", "YG", "GD") & Subclass=="All") %>%
    # group_by(Class, Subclass, Subsubclass)
# aggrs.perf.measure2.2 = subset(stats, Measure==PERFORMANCE.MEASURE & Class!="Model" & Class %in% c("wmedian") & Subsubclass %in% c("GG", "YG", "GD") & Subclass=="All") %>%
    # group_by(Class, Subclass, Subsubclass)

plots = lapply(list(aggrs.perf.measure1.1, aggrs.perf.measure1.2), function(aggrs.perf.measure){
    if (PERFORMANCE.MEASURE.DESC) {
    ...
    ggplot(data=aggrs.perf.measure,
            aes(x=Method, weight=Value, fill=Subclass), environment=environment()) +
    ...
    theme(
          ...
          legend.position="none",
          plot.margin = unit(c(0,0,0,0), "cm")) +
    geom_text(aes(x=Method, y=Value+5,
                  label=round(Value, 1)),
              hjust=0, vjust=0.5, size=2.3) +
    geom_text(aes(x=Method, y=0.05*max(orig.models.perf.measure$Value),
...
tikz(file=paste0("4-",filename,"-all.tex"), sanitize = T, height = 3.6, width = 3.4,
     documentDeclaration = '\\documentclass[conference]{IEEEtran}')
grid.arrange(a0, plots[[1]],plots[[2]], nrow=length(plots)+1, heights=unit(c(0.25, 0.3, 0.3), "npc"))
dev.off()
...
#     mutate(`Cost matrix` =  0.005*`Cost matrix`)
#
# stats = selected.aggrs
# secondaryPerformanceMeasures = c("Sensitivity", "Specificity", "Decisiveness", "Accuracy", "Cost matrix")
#
...
#
# PERF.COLORS = c("#D3DDE2","#B2DF8A","#33A02C","#1B6699","#FF0000")
#
# stats$Measure = factor(stats$Measure,
...
#     facet_grid(.~Subclass+Class, scales="free", space="free")
#
