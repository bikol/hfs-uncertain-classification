...
#  ---- download-keel-dyslexic-dataset ----

temp = tempfile()
content = getBinaryURL(paste(KEEL.DATA.URL),
                       httpheader = c("User-Agent"="R"))
writeBin(content, temp)
unzip(temp,exdir=KEEL.DATABASE.LOCATION)
unlink(temp)
print(paste("Directory downloaded:", KEEL.DATABASE.LOCATION))
