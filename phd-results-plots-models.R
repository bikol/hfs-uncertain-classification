...
b2 = ggplot(data=subset(stats, ((Class=="Model" & Subclass=="Original") | (Method %in%
                                  c("mean_3_(GD)_(cen)_maj", "mean_4_(GG)_(min)_maj", "mean_5_(YG)_(lat)_maj",
                                    "mean_renn_4_(GD)_(cen)_maj", "mean_renn_4_(GG)_(cen)_maj", "mean_renn_4_(YG)_(lat)_maj",
                                    "med_3_(GD)_(cen)_maj", "med_4_(GG)_(cen)_maj", "med_5_(YG)_(lat)_maj",
                                    "med_renn_4_(GD)_(cen)_all", "med_renn_4_(GG)_(cen)_maj", "med_renn_4_(YG)_(cen)_maj"
                                  )
                                                                          ) )
    ...
    scale_x_continuous(breaks=BREAKS.X)
tikz(file=paste0("4-",filename,"-unc.tex"), sanitize = T, height = 2, width = 1.7,
     documentDeclaration = '\\documentclass[conference]{IEEEtran}')
print(b2)
dev.off()


tikz(file=paste0("4-both-unc.tex"), sanitize = T, height = 2, width = 3.4,
     documentDeclaration = '\\documentclass[conference]{IEEEtran}')
grid.arrange(b1,b2, nrow=1, widths=unit(c(0.525,0.45),'npc'))
dev.off()
...
aggrs.perf.measure2.1 = subset(stats, Measure==PERFORMANCE.MEASURE & Class!="Model" & Class %in% c("median") & Subsubclass %in% c("GG", "YG", "GD") & Subclass!="CNN") %>%
    group_by(Class, Subclass, Subsubclass)

plots = lapply(list(aggrs.perf.measure1.1, aggrs.perf.measure2.1), function(aggrs.perf.measure){
    if (PERFORMANCE.MEASURE.DESC) {
